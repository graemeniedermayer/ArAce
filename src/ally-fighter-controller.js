...
export const ally_fighter_controller = (() => {
  ...
  class FlashFXEmitter extends particle_system.ParticleEmitter {
    ...
    CreateParticle_() {
      ...
      // DEMO
      this.light_ = new THREE.PointLight(0xFF8080, 2.0, 2.0, 2.0);
  ...
  class AllyFighterController extends entity.Component {
    constructor(params) {
      super();
      this.params_ = params;
    ...
    InitEntity() {
      ...
      loader.LoadSound('./resources/sounds/', 'propellor.ogg', (s) => {
        group.add(s);

        s.setVolume(.2)
        s.setRefDistance(0.2);
        s.setMaxDistance(2);
        s.setLoop(true);
        s.play();  
      });
    ...
    OnFire_() {
      ...
      const loader = this.FindEntity('loader').GetComponent('LoadController');
      loader.LoadSound('./resources/sounds/', 'cannon.ogg', (s) => {
        const group = this.GetComponent('RenderComponent').group_;
        group.add(s);
        s.setVolume(5.0)
        s.play();  
